...
新版后端的健康接口应返回包含 `apiVersion: 8`、`chunkedUploads: true`、`revisions: true`、`mediaUploads: true` 和 `articleDeletion: true` 的 JSON。若 `/api/version` 或 `/api/capabilities` 返回 404，说明 Railway 仍在运行旧版后端，需要重新部署 `backend` 目录。
