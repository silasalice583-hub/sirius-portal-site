(async function () {
  ...
  function previewFontRules() {
    return Object.entries(fontSizesState).map(([key, rawSize]) => {
      const selector = fontSizeSelectors[key];
      const size = Number(rawSize);
      if (!selector || !Number.isFinite(size) || size < 0 || size > 100) return "";
      return `${selector} { font-size: ${size}px !important; }`;
    }).filter(Boolean).join("\n");
  }
  ...
  function applyFontSizesToPreview() {
    ...
    if (!doc) return;
    let style = doc.getElementById("site-custom-font-sizes");
    if (!style) {
      style = doc.createElement("style");
      style.id = "site-custom-font-sizes";
      doc.head.appendChild(style);
    }
    style.textContent = previewFontRules();
  ...
  function syncComputedFontSizesFromPreview() {
    const doc = frame.contentDocument;
  ...
  function injectPreviewTools() {
    ...
    applyFontSizesToPreview();
    syncComputedFontSizesFromPreview();
