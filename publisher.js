(async function () {
  ...
  if (state.source === "api-error") {
    ...
    backendStatus.textContent = capabilities
      ? `已连接公网内容库 · 数据版本 ${state.revision || 1} · API v${capabilities.apiVersion || "未知"}${capabilities.chunkedUploads ? " · 支持大文章导入" : " · 不支持大文章导入"}${capabilities.mediaUploads ? " · 支持媒体上传" : " · 不支持媒体上传"}${capabilities.articleDeletion ? " · 支持删除文章" : " · 后端需升级以删除文章"}`
          ...
          ${allArticles().map((article) => `
            <tr>
              ...
              <td>
                ...
                <button class="danger-action" type="button" data-delete="${article.id}">删除</button>
              </td>
            </tr>
          `).join("")}
        </tbody>
